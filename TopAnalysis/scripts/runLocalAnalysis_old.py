...
#            condor.write('log        = {0}/$(cfgFile).log\n'.format(FarmDirectory))
            ...
            for method,inF,outF,channel,charge,flav,runSysts,systVar,era,tag,debug in task_list:
                ...
                with open('%s/%s.sh'%(FarmCfgDirectory, cfgFile),'w') as cfg:
                    ...
                    cfg.write('echo "hostname is "`hostname`\n')
                    cfg.write('WORKDIR=`pwd`\n')
                    cfg.write('echo "Working directory is ${WORKDIR}"\n')
                    cfg.write('cd %s\n'%cmsswBase)
                    cfg.write('echo $PATH >&2\n')
                    cfg.write('if [[ ":$PATH:" != *":/cvmfs/cms.cern.ch/common:"* ]]; then export PATH=$PATH:/cvmfs/cms.cern.ch/common/; echo "PATH FIXED >&2"; fi\n') 
                    ...
                    if '/store' in outF:
                        ...
                        cfg.write('sh %s/scripts/EOS_file_copy.sh ${WORKDIR}/%s %s\n' % (PROJECT, localOutF, outF))
                        if isTTbar:
                            cfg.write('sh %s/scripts/EOS_file_copy.sh ${WORKDIR}/migration_%s %s\n' % (PROJECT, localOutF, MigrationFile))
