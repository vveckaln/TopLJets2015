...
def main():
    ...
    plots.signalTitle = signalTitle
    report=''
    fTTJets = ROOT.TFile.Open('%s/MC13TeV_TTJets.root' % ( opt.inDir) )
    fTTJets_method = ROOT.TFile.Open('%s/MC13TeV_TTJets%s.root' % ( opt.inDir, methodtag) )
    print samplesList
    raw_input("press Enter")
    print systTheorSamplesList
    raw_input("press Enter")
...
b
                if method != "nominal" and isExpSyst:
                    file_name = '%s%s/%s.root' % ( opt.inDir, methodtag, sp[0])
                print "file %s theor %u exp %u " % (file_name, isTheorSyst, isExpSyst)
...
#                        if not (ROOT.TString(key).Contains("L_pull_angle_allconst") ):
#                            continue
                        ...
                        for hist in histos:
                            ...
                            if not key in plots : 
                                plots[key] = Plot(key, com = opt.com, signalTitle = signalTitle)
    ...
    if (not opt.outDir): 
        ...
        if method != "nominal":
            outDir = opt.inDir + methodtag + '/plots'
    else:
        outDir = opt.outDir
